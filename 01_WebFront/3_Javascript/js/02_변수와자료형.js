...
console.log(number1); // undefinded
...
number3 = 500; // 기존 변수 number3에 새로운 값을 덮어쓰기(재대입)
console.log("number3 : " +number3)


//------------------------------------------------------------

// const (상수, constant, 항[상] 같은 [수])
...
//const test; // 'const' 선언은 초기화되어야 합니다.
...
// -> 가능하다. 함수레벨 스코프인 var 는 블록을 무시하기 때문

// 그럼 함수 안에서는 어떻게 될까?

// 함수의 선언
function example() { // 함수 형태로 만들어진 코드블록은 함수레벨scope
...
// 즉, 함수 블록 안에서 선언된 var 변수는 함수 안에서만 사용가능
// -> 함수 외부에서 사용 불가
...
// var a; (내부적으로 선언만 되어있는 상태)
console.log(a) // undefined
// -> 호이스팅 되어 var ; 라는 선언이 상단으로 끌어올려짐.
// -> 선언은 되어있으나 값이 초기화 되지 않았기 때문에 undefined 를 출력함
...
// var 와 달리, let/const로 선언된 변수는 초기화가 이루어지기 전에는 사용될 수 없음.

/* 
  *TED(Temporal Dead Zone)
  * 시간적으로 죽은 구역
  * - 코드 실행 과정에서 특정 시점까지 변수가 접근 불가능한 상태를 의미함
  * - Javascript 에서 let/const 키워드로 선언된 변수가 초기화되기 전까지 접근할 수 없는구간을 의미
*/
...
//console.log("numbers 배열값 중 3번째 : " , numbers[3] , typeof numbers[3]);
// undefined - number[3]에는 아무 값도 할당되지 않아 undefined 된다.
...
// 함수를 정의할 때 사용하는 변수 : 매개변수(Parameter)
/* function sumFn(a,b) {
  return a+b
} */
// 동일한 함수
